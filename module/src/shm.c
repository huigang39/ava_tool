...
int
shm_init(struct shm *shm, const struct shm_cfg shm_cfg)
{
    DECL(shm, cfg, lo);
...
size_t
shm_read(struct shm *shm, void *dst, const size_t size)
{
    DECL(shm, lo);

    return spsc_read_buf(lo->spsc, (uint8_t *)lo->addr + sizeof(*lo->spsc), dst, size);
}

size_t
shm_write(struct shm *shm, const void *src, const size_t size)
{
    DECL(shm, lo);

    return spsc_write_buf(lo->spsc, (uint8_t *)lo->addr + sizeof(*lo->spsc), src, size);
}
